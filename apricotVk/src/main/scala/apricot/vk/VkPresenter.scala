...
final class VkPresenter(
  ...
  def createSwapchain(onSwapchainCreatedAction: () => Any): Unit = {
    withStack {
      ...
      conf.presentationMode = if conf.imagesCount > 2 && physicalDevice.getSurfacePresentModesKhr(surface).contains(KHRSurface.VK_PRESENT_MODE_MAILBOX_KHR) then
        KHRSurface.VK_PRESENT_MODE_MAILBOX_KHR else KHRSurface.VK_PRESENT_MODE_FIFO_KHR
      ...
      val surfaceTansform = if currentCapabilities.supportedTransforms.hasFlags(KHRSurface.VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR) then
        KHRSurface.VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR else currentCapabilities.currentTransform

      val newSwapchain = logicalDevice.createSwapchain(surface, conf.imagesCount, firstColorFormat, surfaceFormats.get(0).colorSpace,
        swapchainExtent, conf.usageFlags, VK10.VK_SHARING_MODE_EXCLUSIVE, null, surfaceTansform, conf.compositeAlpha,
        conf.presentationMode, currentSwapchain, clipped = true)
      ...
      _images = (KHRSwapchain.vkGetSwapchainImagesKHR(logicalDevice.unwrap, newSwapchain, _: IntBuffer, _: LongBuffer)).countAndAlloc().toArray()
      _imageViews = new Array[NativeHandle](_images.length)
      val createImageViewInfo = VkFactory.imageView(0, VK10.VK_IMAGE_VIEW_TYPE_2D, firstColorFormat,
        VkFactory.componentMapping(), VkFactory.imageSubresourceRange(VK10.VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1))
      cfor(0, _ < _images.length) {i =>
        createImageViewInfo.image(_images(i))
        _imageViews(i) = (VK10.vkCreateImageView(logicalDevice.unwrap, createImageViewInfo, null, _: LongBuffer))
          .returning(stackCalloc)(VK10.VK_SUCCESS.==, s"failed to create image view for image $i").get(0)
        i + 1  
      }
    }
  ...
  def acquireNextImage(awaitNanos: Long, semaphore: NativeHandle, fence: NativeHandle)(using MemoryStack): Int = {
    var outOfDate = false
    val ord = withStack {
      (KHRSwapchain.vkAcquireNextImageKHR(logicalDevice.unwrap, currentSwapchain, awaitNanos, semaphore, fence, _: IntBuffer))
        .returning(stackMalloc)( {
          ...
          case KHRSwapchain.VK_ERROR_OUT_OF_DATE_KHR => 
            ...
            true
          case _ => false
        }, "failed to acquire image from surface").get
    }
    if (!outOfDate) ord
    else {
  ...
  def present(queue: VkQueue, images: Array[Int], waitSempahores: Array[NativeHandle])(using stack: MemoryStack): Unit = {
    KHRSwapchain.vkQueuePresentKHR(queue, VkFactory.queuePresent(
