  ...
  private class StackSpecificImpl(val vkStack: VulkanGraphicsStack) {
    ...
    def draw(gc: vkStack.VkGraphicsContext): Unit = {
      if (gc.presenter.swapChain != lastSwapChainId) {
        scribe.info(s"swap chained changed from $lastSwapChainId to ${gc.presenter.swapChain}")
    ...
    private def setup(gc: vkStack.VkGraphicsContext): Unit = withStack { stack ?=>
      ...
      val heapIdx = vertexBuffer.findAppropriateHeap(bufferReqs, vkStack.physicalDevice, VK10.VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK10.VK_MEMORY_PROPERTY_HOST_COHERENT_BIT).getOrElse(
        throw new IllegalStateException("host visible heap was not found")
      )
      ...
      vkStack.logicalDevice.flushMemoryRanges(
        alloc[VkMappedMemoryRange](stackCalloc).memory(bufferMemory).offset(0).size(VK10.VK_WHOLE_SIZE).sType(VK10.VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE).buffered(stackCalloc)
      )
      scribe.info("vertex data uploaded")
      ...
      val pipelineCreateInfo = VkFactory
        ...
        .pVertexInputState(
          VkFactory.pipelineVertexInputState(
            VkFactory.vertexBindingDescriptor(0, Sized.of[VertexData].size.toInt, VK10.VK_VERTEX_INPUT_RATE_VERTEX).buffered(stackCalloc),
