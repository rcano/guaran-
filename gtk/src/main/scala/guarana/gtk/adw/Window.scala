...
opaque type Window <: guarana.gtk.Window  = org.gnome.adw.Window & guarana.gtk.Window
object Window extends VarsMap {
  val AdaptivePreview: ExternalVar.Aux[Window, Boolean] = ExternalVar[Window, Boolean]("adaptive-preview", _.getAdaptivePreview(), _.setAdaptivePreview(_), true)
  val Content: ExternalVar.Aux[Window, guarana.gtk.Widget | Null] = ExternalVar[Window, guarana.gtk.Widget | Null]("content", _.getContent().?(guarana.gtk.Widget.wrap), (n, v) => n.setContent(v.?(_.unwrap)), true)

  

  extension (v: Window) {
    def unwrap: org.gnome.adw.Window = v

    def adaptivePreview: Var.Aux[Boolean, v.type] = guarana.gtk.adw.Window.AdaptivePreview.asInstanceOf[Var.Aux[Boolean, v.type]]
    def content: Var.Aux[guarana.gtk.Widget | Null, v.type] = guarana.gtk.adw.Window.Content.asInstanceOf[Var.Aux[guarana.gtk.Widget | Null, v.type]]
  ...
  def wrap(v: org.gnome.adw.Window): Window = 
  ...
  def uninitialized(defaultHeight: Opt[Int], defaultWidth: Opt[Int], focusWidget: Opt[guarana.gtk.Widget], fullscreened: Opt[Boolean], maximized: Opt[Boolean], startupId: Opt[java.lang.String], cssName: Opt[java.lang.String], heightRequest: Opt[Int], widthRequest: Opt[Int], accessibleRole: Opt[org.gnome.gtk.AccessibleRole]): Window = {
    val res = {
      val res = org.gnome.adw.Window.builder()
  ...
  def apply(
    ...
    adaptivePreview: Opt[Binding[Boolean]] = UnsetParam,
    ...
    guarana.gtk.adw.Window.init(res)
    ifSet(adaptivePreview, res.adaptivePreview := _)
