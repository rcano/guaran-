...
opaque type ApplicationWindow <: guarana.gtk.Window  = org.gnome.adw.ApplicationWindow & guarana.gtk.Window
object ApplicationWindow extends VarsMap {
  val AdaptivePreview: ExternalVar.Aux[ApplicationWindow, Boolean] = ExternalVar[ApplicationWindow, Boolean]("adaptive-preview", _.getAdaptivePreview(), _.setAdaptivePreview(_), true)
  val Content: ExternalVar.Aux[ApplicationWindow, guarana.gtk.Widget | Null] = ExternalVar[ApplicationWindow, guarana.gtk.Widget | Null]("content", _.getContent().?(guarana.gtk.Widget.wrap), (n, v) => n.setContent(v.?(_.unwrap)), true)

  

  extension (v: ApplicationWindow) {
    def unwrap: org.gnome.adw.ApplicationWindow = v

    def adaptivePreview: Var.Aux[Boolean, v.type] = guarana.gtk.adw.ApplicationWindow.AdaptivePreview.asInstanceOf[Var.Aux[Boolean, v.type]]
    def content: Var.Aux[guarana.gtk.Widget | Null, v.type] = guarana.gtk.adw.ApplicationWindow.Content.asInstanceOf[Var.Aux[guarana.gtk.Widget | Null, v.type]]
  ...
  def wrap(v: org.gnome.adw.ApplicationWindow): ApplicationWindow = 
  ...
  def uninitialized(defaultHeight: Opt[Int], defaultWidth: Opt[Int], focusWidget: Opt[guarana.gtk.Widget], fullscreened: Opt[Boolean], maximized: Opt[Boolean], startupId: Opt[java.lang.String], cssName: Opt[java.lang.String], heightRequest: Opt[Int], widthRequest: Opt[Int], accessibleRole: Opt[org.gnome.gtk.AccessibleRole]): ApplicationWindow = {
    val res = {
      val res = org.gnome.adw.ApplicationWindow.builder()
  ...
  def apply(
    ...
    guarana.gtk.adw.ApplicationWindow.init(res)
