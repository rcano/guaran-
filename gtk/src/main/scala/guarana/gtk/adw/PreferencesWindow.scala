...
opaque type PreferencesWindow <: guarana.gtk.Window  = org.gnome.adw.PreferencesWindow & guarana.gtk.Window
object PreferencesWindow extends VarsMap {
  @deprecated("", "") val CanNavigateBack: ExternalVar.Aux[PreferencesWindow, Boolean] = ExternalVar[PreferencesWindow, Boolean]("can-navigate-back", _.getCanNavigateBack(), _.setCanNavigateBack(_), true)
  @deprecated("", "") val SearchEnabled: ExternalVar.Aux[PreferencesWindow, Boolean] = ExternalVar[PreferencesWindow, Boolean]("search-enabled", _.getSearchEnabled(), _.setSearchEnabled(_), true)

  

  extension (v: PreferencesWindow) {
    def unwrap: org.gnome.adw.PreferencesWindow = v

    @deprecated("", "") def canNavigateBack: Var.Aux[Boolean, v.type] = guarana.gtk.adw.PreferencesWindow.CanNavigateBack.asInstanceOf[Var.Aux[Boolean, v.type]]
    @deprecated("", "") def searchEnabled: Var.Aux[Boolean, v.type] = guarana.gtk.adw.PreferencesWindow.SearchEnabled.asInstanceOf[Var.Aux[Boolean, v.type]]
  ...
  def wrap(v: org.gnome.adw.PreferencesWindow): PreferencesWindow = 
    val res = v.asInstanceOf[PreferencesWindow]
    
    res

  def init(v: PreferencesWindow): Unit = {
    guarana.gtk.Window.init(v)
    connectVarsListener(v)
    
  }
  def uninitialized(visiblePage: Opt[guarana.gtk.Widget], visiblePageName: Opt[java.lang.String], defaultHeight: Opt[Int], defaultWidth: Opt[Int], focusWidget: Opt[guarana.gtk.Widget], fullscreened: Opt[Boolean], maximized: Opt[Boolean], startupId: Opt[java.lang.String], cssName: Opt[java.lang.String], heightRequest: Opt[Int], widthRequest: Opt[Int], accessibleRole: Opt[org.gnome.gtk.AccessibleRole]): PreferencesWindow = {
    val res = {
      val res = org.gnome.adw.PreferencesWindow.builder()
      ifSet(visiblePage, v => res.setVisiblePage(v.unwrap))
      ifSet(visiblePageName, v => res.setVisiblePageName(v))
  ...
  def apply(
    ...
    searchEnabled: Opt[Binding[Boolean]] = UnsetParam,
  ...
  ): VarContextAction[PreferencesWindow] = {
    val res = uninitialized(visiblePage, visiblePageName, defaultHeight, defaultWidth, focusWidget, fullscreened, maximized, startupId, cssName, heightRequest, widthRequest, accessibleRole)
    guarana.gtk.adw.PreferencesWindow.init(res)
    ...
    ifSet(searchEnabled, res.searchEnabled := _)
