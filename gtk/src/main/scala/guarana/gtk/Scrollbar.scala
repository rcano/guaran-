...
object Scrollbar extends VarsMap {
  ...
  def _wrap(v: org.gnome.gtk.Scrollbar): Scrollbar = {
    v.asInstanceOf
  }
  def init(v: Scrollbar): ToolkitAction[Toolkit, Unit] = {
    Widget.init(v)
