...
import java.util.function.{BiConsumer, BiFunction, Consumer, Function, IntFunction, IntPredicate}
...
/**
 * Literal port of `org.agrona.collections.Int2ObjectHashMap`.
 *
...
class Int2ObjectHashMap[V](
  initialCapacity: Int = Int2ObjectHashMap.MIN_CAPACITY,
  loadFactorCtor: Float = Hashing.DEFAULT_LOAD_FACTOR,
  shouldAvoidAllocationCtor: Boolean = true)
  extends JMap[Integer, V]
{
  import Int2ObjectHashMap._
  ...
  private var keys: Array[Int] = scala.compiletime.uninitialized
   ...
   */
  def this(mapToCopy: Int2ObjectHashMap[V]) =
  {
   ...
   * {@inheritDoc}
   */
  override def forEach(action: BiConsumer[? >: Integer, ? >: V]): Unit =
  {
    forEachInt(new IntObjConsumer[V]
    {
      override def accept(key: Int, value: V): Unit = action.accept(key, value)
    })
  }

  /**
   * Use `forEachInt(IntObjConsumer)` instead.
   *
   * @param consumer a callback called for each key/value pair in the map.
   * @see #forEachInt
   * @deprecated Use `forEachInt(IntObjConsumer)` instead.
   */
  @deprecated("Use forEachInt(IntObjConsumer) instead.", "port")
  def intForEach(consumer: IntObjConsumer[V]): Unit = forEachInt(consumer)
   ...
   * @param consumer a callback called for each key/value pair in the map.
   */
  def forEachInt(consumer: IntObjConsumer[V]): Unit =
  {
   ...
   * {@inheritDoc}
   */
  override def containsKey(key: Any): Boolean = containsKey(key.asInstanceOf[Integer].intValue())
   ...
   * {@inheritDoc}
   */
  override def get(key: Any): V = get(key.asInstanceOf[Integer].intValue())
   ...
   */
  def get(key: Int): V = unmapNullValue(getMapped(key))
   ...
   */
  def getOrDefault(key: Int, defaultValue: V): V =
  {
   ...
   */
  protected def getMapped(key: Int): V =
  {
    val keys = this.keys
    val values = this.valuesArray
    @DoNotSub val mask = values.length - 1
   ...
   * {@inheritDoc}
   */
  override def computeIfAbsent(key: Integer, mappingFunction: Function[? >: Integer, ? <: V]): V =
    computeIfAbsent(key.intValue(), new IntFunction[V]
    {
      override def apply(k: Int): V = mappingFunction.apply(k)
    })
   ...
   */
  def computeIfAbsent(key: Int, mappingFunction: IntFunction[? <: V]): V =
  {
  ...
  private def finishComputeIfAbsent(
    ...
    mappingFunction: IntFunction[? <: V]): V =
  {
  ...
  override def computeIfPresent(
    key: Integer, remappingFunction: BiFunction[? >: Integer, ? >: V, ? <: V]): V =
    computeIfPresent(key.intValue(), new IntObjectToObjectFunction[V, V]
    {
      override def apply(k: Int, value: V): V = remappingFunction.apply(k, value)
    })
  ...
  def computeIfPresent(
    key: Int, remappingFunction: IntObjectToObjectFunction[? >: V, ? <: V]): V =
  {
   ...
   * {@inheritDoc}
   */
  override def compute(key: Integer, remappingFunction: BiFunction[? >: Integer, ? >: V, ? <: V]): V =
    compute(key.intValue(), new IntObjectToObjectFunction[V, V]
    {
      override def apply(k: Int, value: V): V = remappingFunction.apply(k, value)
    })
   ...
   * @return the new value associated with the specified key, or `null` if none.
   */
  def compute(key: Int, remappingFunction: IntObjectToObjectFunction[? >: V, ? <: V]): V =
  {
    Objects.requireNonNull(remappingFunction)
    val keys = this.keys
    val values = this.valuesArray
  ...
  private def finishCompute(
    ...
    key: Int,
    mappedvalue: Object,
    remappingFunction: IntObjectToObjectFunction[? >: V, ? <: V]): V =
  {
   ...
   * {@inheritDoc}
   */
  override def merge(key: Integer, value: V, remappingFunction: BiFunction[? >: V, ? >: V, ? <: V]): V =
    merge(key.intValue(), value, remappingFunction)
   ...
   */
  def merge(key: Int, value: V, remappingFunction: BiFunction[? >: V, ? >: V, ? <: V]): V =
  {
   ...
   * {@inheritDoc}
   */
  override def put(key: Integer, value: V): V = put(key.intValue(), value)
   ...
   * {@inheritDoc}
   */
  override def remove(key: Any): V = remove(key.asInstanceOf[Integer].intValue())
   ...
   * {@inheritDoc}
   */
  override def remove(key: Any, value: Any): Boolean = remove(key.asInstanceOf[Integer].intValue(), value.asInstanceOf[V])
   ...
   * {@inheritDoc}
   */
  override def putAll(map: JMap[? <: Integer, ? <: V]): Unit =
  {
   ...
   */
  def putAll(map: Int2ObjectHashMap[? <: V]): Unit =
  {
    val iterator: map.EntryIterator = map.entrySet().iterator()
    while (iterator.hasNext)
    {
      iterator.findNext()
      put(iterator.getIntKey(), iterator.getValue().asInstanceOf[V])
    }
  }
   ...
   */
  def putIfAbsent(key: Int, value: V): V =
  {
    val v = mapNullValue(value.asInstanceOf[AnyRef])
    Objects.requireNonNull(v, "value cannot be null")
  ...
  override def toString(): String =
  {
    ...
    {
      entryIterator.next()
      sb.append(entryIterator.getIntKey()).append('=').append(unmapNullValue(entryIterator.getValue().asInstanceOf[Object]))
    ...
    o match
    {
      case that: JMap[_, _] =>
        ...
        while (i < length)
        {
          ...
          if (null != thisValue)
          {
            val thatValue = that.asInstanceOf[JMap[Any, Any]].get(Integer.valueOf(keys(i)))
   ...
   */
  def replace(key: Int, oldValue: V, newValue: V): Boolean =
  {
   ...
   * {@inheritDoc}
   */
  override def replaceAll(function: BiFunction[? >: Integer, ? >: V, ? <: V]): Unit =
  {
    replaceAllInt(new IntObjectToObjectFunction[V, V]
    {
      override def apply(k: Int, value: V): V = function.apply(k, value)
    })
  }
   ...
   */
  def replaceAllInt(function: IntObjectToObjectFunction[? >: V, ? <: V]): Unit =
  {
  ...
  private def rehash(@DoNotSub newCapacity: Int): Unit =
  {
    ...
    val tempKeys = new Array[Int](newCapacity)
  ...
  final class KeySet extends AbstractSet[Integer]
  {
     ...
     * {@inheritDoc}
     */
    @DoNotSub override def size(): Int = Int2ObjectHashMap.this.size()

    /**
     * {@inheritDoc}
     */
    override def contains(o: Any): Boolean = Int2ObjectHashMap.this.containsKey(o)
     ...
     */
    def contains(key: Int): Boolean = Int2ObjectHashMap.this.containsKey(key)

    /**
     * {@inheritDoc}
     */
    override def remove(o: Any): Boolean = null != Int2ObjectHashMap.this.remove(o)
     ...
     */
    def remove(key: Int): Boolean = null != Int2ObjectHashMap.this.remove(key)

    /**
     * {@inheritDoc}
     */
    override def clear(): Unit = Int2ObjectHashMap.this.clear()
  ...
  final class ValueCollection extends AbstractCollection[V]
  {
     ...
     * {@inheritDoc}
     */
    override def contains(o: Any): Boolean = Int2ObjectHashMap.this.containsValue(o)
    ...
    {
      @DoNotSub var remaining = Int2ObjectHashMap.this._size

      val values = Int2ObjectHashMap.this.valuesArray
      @DoNotSub var i = 0
      @DoNotSub val length = values.length
      while (remaining > 0 && i < length)
      {
   ...
   * allocation.
   */
  final class EntrySet extends AbstractSet[JEntry[Integer, V]]
  {
    ...
    override def contains(o: Any): Boolean =
    {
      o match
      {
        case entry: JEntry[_, _] =>
          val key = entry.getKey().asInstanceOf[Integer].intValue()
          val value = getMapped(key)
     ...
     * @return `true` if at least one key was removed.
     */
    def removeIfInt(filter: IntObjPredicate[V]): Boolean =
    {
      var removed = false
      val iterator = this.iterator()
      while (iterator.hasNext)
  ...
  private[collections] abstract class AbstractIterator[T] extends java.util.Iterator[T]
  {
    ...
    private[Int2ObjectHashMap] var isPositionValid: Boolean = false
     ...
     */
    protected[Int2ObjectHashMap] final def findNext(): Unit =
    {
      if (!hasNext())
      {
    ...
    private[Int2ObjectHashMap] def reset(): Unit =
    {
      _remaining = Int2ObjectHashMap.this._size
      val values = Int2ObjectHashMap.this.valuesArray
   ...
   */
  final class KeyIterator extends AbstractIterator[Integer]
  {
    /**
     * {@inheritDoc}
     */
    override def next(): Integer = nextInt()
  ...
  final class EntryIterator
    extends AbstractIterator[JEntry[Integer, V]]
    with JEntry[Integer, V]
  {
    /**
     * {@inheritDoc}
     */
    override def next(): JEntry[Integer, V] =
    {
      findNext()
    ...
    private[collections] def allocateDuplicateEntry(): JEntry[Integer, V] =
      new MapEntry(getIntKey(), getValue())

    /**
     * {@inheritDoc}
     */
    override def getKey(): Integer = getIntKey()
     ...
     * @return key.
     */
    def getIntKey(): Int = keys(position())
     ...
     */
    final class MapEntry(k: Int, v: V) extends JEntry[Integer, V]
    {
      /**
       * {@inheritDoc}
       */
      override def getKey(): Integer = Integer.valueOf(k)
       ...
       * {@inheritDoc}
       */
      override def setValue(value: V): V = Int2ObjectHashMap.this.put(k, value)

      /**
       * {@inheritDoc}
       */
      @DoNotSub override def hashCode(): Int = Integer.hashCode(k) ^ (if (null != v) v.hashCode() else 0)
