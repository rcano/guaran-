...
object IntHashSet {
  ...
  private[IntHashSet] val MISSING_VALUE = -1
...
/** Open-addressing with linear-probing expandable hash set. Allocation free in steady state use when expanded. <p> By storing elements as
  * int primitives this significantly reduces memory consumption compared with Java's builtin <code>HashSet&ltInteger&gt</code>. It
  * implements <code>Set&ltInteger&gt</code> for convenience, but calling functionality via those methods can add boxing overhead to your
...
class IntHashSet(
    val proposedCapacity: Int = IntHashSet.DEFAULT_INITIAL_CAPACITY,
  ...
  private[IntHashSet] var containsMissingValue = false
  ...
  private[IntHashSet] var values: Array[Int] = uninitialized
  private var _iterator: IntIterator | Null = null
  ...
  /** Get the actual threshold which when reached the map will resize. This is a function of the current capacity and load factor.
    ...
    */
  def resizeThreshold: Int = _resizeThreshold

  /** {@inheritDoc}
    */
  override def add(value: java.lang.Integer): Boolean = add(value.intValue)
  ...
  override def remove(value: Object): Boolean = {
    return value.isInstanceOf[java.lang.Integer] && remove(value.asInstanceOf[java.lang.Integer].intValue())
  }
  ...
  override def contains(value: Object): Boolean = value match
    case i: java.lang.Integer => contains(i.intValue)
    case _ => false
  ...
  /** {@inheritDoc}
    */
  override def addAll(coll: java.util.Collection[? <: Integer]): Boolean = {
    var added = false
  ...
  /** IntHashSet specialised variant of {this#containsAll(Collection)}.
    *
    * @param other
    *   int hash set to compare against.
    * @return
    *   true if every element in other is in this.
    */
  def containsAll(other: IntHashSet): Boolean = {
    var i = -1
  ...
  /** Fast Path set difference for comparison with another IntHashSet. <p> <b>Note:</b> garbage free in the identical case, allocates
    * otherwise.
    *
    * @param other
  ...
  def difference(other: IntHashSet): IntHashSet = {
    var difference: IntHashSet | Null = null
    ...
    return difference.asInstanceOf[IntHashSet]
  }
  ...
  private def copyValues(arrayCopy: Array[Object]): Unit = {
    ...
    for (value <- _values)
    {
      if (MISSING_VALUE != value) {
        i += 1
        arrayCopy(i) = value: java.lang.Integer
      }
    }

    if (containsMissingValue) {
      arrayCopy(sizeOfArrayValues) = MISSING_VALUE: java.lang.Integer
    }
  }
  ...
  final class IntIterator extends java.util.Iterator[java.lang.Integer], java.io.Serializable {
    ...
    def reset(): IntIterator = {
      ...
      {
        var i = -1
        while ({ i += 1; i < length && values(i) != IntHashSet.MISSING_VALUE }) ()
      }
    ...
    def position(values: Array[Int]): Int = {
