...
sealed trait EventIterator[-T] {
  def step(t: T): VarContextAction[Option[EventIterator[T]]]

  def foreach[U <: T](f: VarContextAction[U => Any]): EventIterator[U]
  def forsome[U <: T](f: VarContextAction[PartialFunction[U, Any]]): EventIterator[U]
  def filter[U <: T](pred: VarContextAction[U => Boolean]): EventIterator[U]
  def take(size: Int): EventIterator[T]
  def takeWhile[U <: T](pred: VarContextAction[U => Boolean]): EventIterator[U]
  def drop(size: Int): EventIterator[T]
  def dropWhile[U <: T](pred: VarContextAction[U => Boolean]): EventIterator[U]
}
object EventIterator extends EventIterator[Any] {
  def step(t: Any) = Some(this)

  private val step0 = EventIteratorImpl[Any](Seq.empty)
  def foreach[U <: Any](f: VarContextAction[U => Any]): EventIterator[U] = step0.foreach(f)
  def forsome[U <: Any](f: VarContextAction[PartialFunction[U, Any]]): EventIterator[U] = step0.forsome(f)
  def filter[U <: Any](pred: VarContextAction[U => Boolean]): EventIterator[U] = step0.filter(pred)
  def take(size: Int) = step0.take(size)
  def takeWhile[U <: Any](pred: VarContextAction[U => Boolean]): EventIterator[U] = step0.takeWhile(pred)
  def drop(size: Int) = step0.drop(size)
  def dropWhile[U <: Any](pred: VarContextAction[U => Boolean]): EventIterator[U] = step0.dropWhile(pred)
