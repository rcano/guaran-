...
object Frame extends VarsMap {
  ...
  def apply(
    ...
    ifSet(contentPane, Frame.ops.extension_contentPane(res) := _)
    ifSet(cursor, Node.ops.extension_cursor(res) := _)
    ifSet(defaultCloseOperation, Frame.ops.extension_defaultCloseOperation(res) := _)
    ifSet(enabled, Node.ops.extension_enabled(res) := _)
    ifSet(extendedState, Frame.ops.extension_extendedState(res) := _)
    ...
    ifSet(glassPane, Frame.ops.extension_glassPane(res) := _)
    ifSet(iconImages, WindowBase.ops.extension_iconImages(res) := _)
    ifSet(layeredPane, Frame.ops.extension_layeredPane(res) := _)
    ifSet(locationByPlatform, WindowBase.ops.extension_locationByPlatform(res) := _)
    ifSet(maxSize, Node.ops.extension_maxSize(res) := _)
    ifSet(maximizedBounds, Frame.ops.extension_maximizedBounds(res) := _)
    ifSet(menuBar, Frame.ops.extension_menuBar(res) := _)
    ...
    ifSet(state, Frame.ops.extension_state(res) := _)
    ifSet(title, Frame.ops.extension_title(res) := _)
    ifSet(tpe, WindowBase.ops.extension_tpe(res) := _)
    ifSet(transferHandler, Frame.ops.extension_transferHandler(res) := _)
    ifSet(undecorated, Frame.ops.extension_undecorated(res) := _)
