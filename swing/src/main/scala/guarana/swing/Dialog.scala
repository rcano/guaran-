...
object Dialog extends VarsMap {
  ...
  def apply(
    ...
    ifSet(contentPane, Dialog.ops.extension_contentPane(res) := _)
    ifSet(cursor, Node.ops.extension_cursor(res) := _)
    ifSet(defaultCloseOperation, Dialog.ops.extension_defaultCloseOperation(res) := _)
    ...
    ifSet(resizable, Dialog.ops.extension_resizable(res) := _)
    ifSet(root, WindowBase.ops.extension_root(res) := _)
    ifSet(shape, WindowBase.ops.extension_shape(res) := _)
    ifSet(title, Dialog.ops.extension_title(res) := _)
    ifSet(tpe, WindowBase.ops.extension_tpe(res) := _)
    ifSet(transferHandler, Dialog.ops.extension_transferHandler(res) := _)
    ifSet(undecorated, Dialog.ops.extension_undecorated(res) := _)
