...
/** Since there's no way in scala to abstract over method parameters, and the builders guaraná provides are based on factory methods, we
  * need a way to generate the node mappings trying to avoid manual coding mistakes. This class is used to encode the properties per type:
  * well: inheritance, so that the code for each node type can be generated.
  */
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "componentPopupMenu",
          "PopupMenu | Null",
          "_.getComponentPopupMenu.asInstanceOf",
          "(c, p) => c.setComponentPopupMenu(p.asInstanceOf)"
        ),
      ...
      initExtra = Seq("v.putClientProperty(InitializedKey, true)"),
      companionObjectExtras = Seq(
...
case object WindowBase
    extends NodeDescr(
      ...
      uninitExtraParams =
        Seq(Parameter("parent", "WindowBase | Null = null", "parent"), Parameter("gc", "GraphicsConfiguration | Null = null", "gc")),
    )
...
case object Frame
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "menuBar",
          "guarana.swing.MenuBar | Null",
          "_.getJMenuBar.?(guarana.swing.MenuBar.wrap)",
          "(f, m) => f.setJMenuBar(m.?(_.unwrap))"
        ),
        SwingProp("contentPane", "java.awt.Container", "_.getContentPane().nn", "_.setContentPane(_)"),
      ...
      companionObjectExtras = Seq("""|def mainWindow: Option[Frame] =
                                     |  java.awt.Frame.getFrames match
                                     |    case Array(jf: JFrame, _*) => Some(wrap(jf))
                                     |    case _ => None""".stripMargin)
    )
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "items",
          "Seq[MenuItem]",
          "c => (0 until c.getComponentCount).map(c.getComponent(_).asInstanceOf[MenuItem])",
          "(c, items) => { c.removeAll(); items foreach (i => c.add(i.unwrap)) }"
        ),
      ),
      opsExtra = Seq(
...
case object RadioButtonMenuItem
    extends NodeDescr(
      "RadioButtonMenuItem",
...
case object ListView
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "cellRenderer",
          "javax.swing.ListCellRenderer[_ >: E]",
          "_.getCellRenderer.nn",
          "(l, c) => l.setCellRenderer(c.asInstanceOf)",
          overrideTpeInStaticPos = Some("javax.swing.ListCellRenderer[_]")
        ),
        SwingProp("dragEnabled", "Boolean", "_.getDragEnabled", "_.setDragEnabled(_)"),
        SwingProp("dropMode", "javax.swing.DropMode | Null"),
        ...
        SwingProp(
          ...
          "_.getModel.nn",
          "(l, m) => l.setModel(m.asInstanceOf)",
          overrideTpeInStaticPos = Some("javax.swing.ListModel[_]")
        ),
        SwingProp(
          "prototypeCellValue",
          "E | Null",
          "_.getPrototypeCellValue",
          "(l, p) => l.setPrototypeCellValue(p.asInstanceOf)",
          overrideTpeInStaticPos = Some("_")
        ),
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "content",
          "Node | Null",
          "_.getViewport.nn.getView.asInstanceOf[Node | Null]",
          "(s, n) => s.setViewportView(if (n != null) n.unwrap else null)"
        ),
        ...
        SwingProp(
          "topLeftCorner",
          "Node | Null",
          "_.getCorner(ScrollPaneConstants.UPPER_LEFT_CORNER).asInstanceOf[Node | Null]",
          "(s, n) => s.setCorner(ScrollPaneConstants.UPPER_LEFT_CORNER, if (n != null) n.unwrap else null)"
        ),
        SwingProp(
          "topRightCorner",
          "Node | Null",
          "_.getCorner(ScrollPaneConstants.UPPER_RIGHT_CORNER).asInstanceOf[Node | Null]",
          "(s, n) => s.setCorner(ScrollPaneConstants.UPPER_RIGHT_CORNER, if (n != null) n.unwrap else null)"
        ),
        SwingProp(
          "botLeftCorner",
          "Node | Null",
          "_.getCorner(ScrollPaneConstants.LOWER_LEFT_CORNER).asInstanceOf[Node | Null]",
          "(s, n) => s.setCorner(ScrollPaneConstants.LOWER_LEFT_CORNER, if (n != null) n.unwrap else null)"
        ),
        SwingProp(
          "botRightCorner",
          "Node | Null",
          "_.getCorner(ScrollPaneConstants.LOWER_RIGHT_CORNER).asInstanceOf[Node | Null]",
          "(s, n) => s.setCorner(ScrollPaneConstants.LOWER_RIGHT_CORNER, if (n != null) n.unwrap else null)"
        ),
      ),
      opsExtra = Seq(
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "horizontal",
          "Boolean",
          "_.getOrientation == JSplitPane.HORIZONTAL_SPLIT",
          "(s, h) => s.setOrientation(if (h) JSplitPane.HORIZONTAL_SPLIT else JSplitPane.VERTICAL_SPLIT)"
        ),
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "selectedComponent",
          "Option[Node]",
          "_.getSelectedComponent.toOption.asInstanceOf",
          "(p, n) => p.setSelectedComponent(n.fold(null)(_.unwrap))"
        ),
        SwingProp("selectedIndex", "Int"),
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          ...
          "_.getModel.nn",
          "(c, m) => c.setModel(m.asInstanceOf)",
          overrideTpeInStaticPos = Some("javax.swing.ComboBoxModel[?]")
        ),
        SwingProp("popupVisible", "Boolean"),
        SwingProp(
          "prototypeDisplayValue",
          "E",
          "_.getPrototypeDisplayValue.asInstanceOf",
          "(c, v) => c.setPrototypeDisplayValue(v.asInstanceOf)",
          overrideTpeInStaticPos = Some("Any")
        ),
        SwingProp(
          "renderer",
          "javax.swing.ListCellRenderer[_ >: E]",
          "_.getRenderer.nn",
          "(c, r) => c.setRenderer(r.asInstanceOf)",
          overrideTpeInStaticPos = Some("javax.swing.ListCellRenderer[?]")
        ),
        SwingProp("selectedIndex", "Int"),
        SwingProp(
          "selectedItem",
          "Option[E]",
          "{c => val v = c.getSelectedItem; if (v != null) Some(v) else None}",
          "_.setSelectedItem(_)",
          overrideTpeInStaticPos = Some("Option[?]")
        ),
        SwingProp(
          "items",
          "Seq[E]",
          "{c => val m = c.getModel.nn; (0 until m.getSize).map(m.getElementAt(_).asInstanceOf[Object])}",
          "(c, i) => c.setModel(DefaultComboBoxModel(i.toArray.asInstanceOf[Array[Object | Null]]).asInstanceOf)",
          overrideTpeInStaticPos = Some("Seq[Any]")
        ),
      ),
      opsExtra = Seq(
    ...
    extends NodeDescr(
      ...
      props = Seq(
        ...
        SwingProp(
          "horizontal",
          "Boolean",
          "_.getOrientation == SwingConstants.HORIZONTAL",
          "(s, h) => s.setOrientation(if (h) SwingConstants.HORIZONTAL else SwingConstants.VERTICAL)"
        )
      )
    )
...
def genCode(n: NodeDescr): String = {
  ...
  val allVars: Vector[(NodeDescr, Property)] = Iterator
    .unfold(Seq(n)) {
      case Seq() => None
      case parents =>
        val allParentVars = parents.flatMap(p =>
          p.props.filterNot(prop => seenVars(prop.name) || prop.visibility.exists(_ `startsWith` "private")).map(p -> _)
        )
  ...
  s"""opaque type ${n.name}$tpeParamsDecls $upperBounds $lowerBounds = ${(n.underlying +: n.upperBounds).mkString(" & ")}
    ...
    .mkString("\n      ")}
     ...
     |      ${sortedEmitters
    .map(e => s"def ${e.name}: Emitter.Aux[${e.tpe}, v.type] = ${n.name}.${e.name.capitalize}.forInstance(v)")
    .mkString("\n      ")}
