...
object Window {
  ...
  private val ContentOrientation: ExternalVar.Aux[Window, io.qt.core.Qt.ScreenOrientation] = ExternalVar[Window, io.qt.core.Qt.ScreenOrientation]("contentOrientation", _.contentOrientation().nn, (_, _) => (), true)
  ...
  val Modality: ExternalVar.Aux[Window, io.qt.core.Qt.WindowModality] = ExternalVar[Window, io.qt.core.Qt.WindowModality]("modality", _.modality().nn, _.setModality(_), true)
  ...
  val Visibility: ExternalVar.Aux[Window, io.qt.gui.QWindow.Visibility] = ExternalVar[Window, io.qt.gui.QWindow.Visibility]("visibility", _.visibility().nn, _.setVisibility(_), true)
  ...
  def init(v: Window): Unit = {
    ...
    Toolkit.connectVar(ContentOrientation.forInstance[v.type], v.contentOrientationChanged.nn)
    Toolkit.connectVar(Height.forInstance[v.type], v.heightChanged.nn)
    Toolkit.connectVar(MaximumHeight.forInstance[v.type], v.maximumHeightChanged.nn)
    Toolkit.connectVar(MaximumWidth.forInstance[v.type], v.maximumWidthChanged.nn)
    Toolkit.connectVar(MinimumHeight.forInstance[v.type], v.minimumHeightChanged.nn)
    Toolkit.connectVar(MinimumWidth.forInstance[v.type], v.minimumWidthChanged.nn)
    Toolkit.connectVar(Modality.forInstance[v.type], v.modalityChanged.nn)
    Toolkit.connectVar(Opacity.forInstance[v.type], v.opacityChanged.nn)
    Toolkit.connectVar(TransientParent.forInstance[v.type], v.transientParentChanged.nn)
    Toolkit.connectVar(Visibility.forInstance[v.type], v.visibilityChanged.nn)
    Toolkit.connectVar(Visible.forInstance[v.type], v.visibleChanged.nn)
    Toolkit.connectVar(Width.forInstance[v.type], v.widthChanged.nn)
    Toolkit.connectVar(X.forInstance[v.type], v.xChanged.nn)
    Toolkit.connectVar(Y.forInstance[v.type], v.yChanged.nn)
