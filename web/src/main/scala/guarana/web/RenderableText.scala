...
/** `RenderableText[A]` is evidence that you can convert a value of type A to a string for the purpose of rendering it as a TextNode.
  ...
  * into [[com.raquo.laminar.nodes.TextNode]]), and will accept your values as a valid [[Modifier]], and in `child.text <--`.
  *
  * See also – [[RenderableNode]]
  * Note: Taken from the great Laminar
  */
@implicitNotFound(
...
trait RenderableText[-TextLike] {
  ...
  extension (textLike: TextLike) def renderText(): String = asString(textLike)
}
